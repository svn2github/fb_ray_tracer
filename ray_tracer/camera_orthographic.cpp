...
namespace ray_tracer {
	...
	colorRGB camera_orthographic::render_scene(double x, double y, int w, int h, world *world_ptr) {
		double u = world_ptr->get_view_plane()->compute_u(x, w);
		double v = world_ptr->get_view_plane()->compute_v(y, h);
		
		return camera::render_scene(eye + u * axis_u + v * axis_v, -axis_w, world_ptr);
	}
}