...
/**
	Caution: Only works on 32-bit processor.
**/
...
namespace ray_tracer {
	...
	bool image_bmp::create() {
		...
		actual_size = *(int *)(bmp_head + 0x2);
		pixel_array_offset = *(int *)(bmp_head + 0xa);
		...
		data_ptr = (char *)bmp_data_ptr;

		/* Parse info header. */
		int bmp_header_size = *(int *)data_ptr;
		if (bmp_header_size < 40) return false; // BITMAPCOREHEADER is not supported
		bmp_width = *(int *)(data_ptr + 0x4);
		if (bmp_width <= 0) return false; // Negative value is forbidden
		bmp_height = *(int *)(data_ptr + 0x8);
		if (bmp_height == 0) return false; // Negative value is allowed
		bmp_color_depth = (*(int *)(data_ptr + 0xe)) & 0xffff;
		bmp_compression = *(int *)(data_ptr + 0x10);
		if (bmp_compression != BI_RGB &&
			bmp_compression != BI_BITFIELDS) return false; // Other compression modes are not supported
		bmp_palette_size = *(int *)(data_ptr + 0x20);
		...
		if (bmp_compression == BI_RGB) {
			...
			bmp_red_mask =   *(int *)(data_ptr + 0x28);
			bmp_green_mask = *(int *)(data_ptr + 0x2c);
			bmp_blue_mask =  *(int *)(data_ptr + 0x30);
		...
		if (bmp_palette_size) {
			int *bmp_palette_offset = (int *)(data_ptr + bmp_header_size), r, g, b;
			for (int i = 0; i < bmp_palette_size; ++i) {
				apply_color_mask(*bmp_palette_offset, r, g, b);
				bmp_palette_r.push_back(r);
				bmp_palette_g.push_back(g);
				bmp_palette_b.push_back(b);
				bmp_palette_offset += 1;
			}
		}
	...
	int image_bmp::width() const {
		return bmp_width;
	}

	int image_bmp::height() const {
		return bmp_height > 0 ? bmp_height : -bmp_height;
	}

	int image_bmp::color(int x, int y) const {
		/* Calculate location of pixel in DIB. */
		char *ptr = (char *)bmp_pixel_array_ptr + bmp_row_size * (bmp_height > 0 ? bmp_height - y - 1 : y);
		...
		if (bmp_color_depth <= 8) {
			ret = (*ptr >> bit_offset) & ((1 << bmp_color_depth) - 1);
